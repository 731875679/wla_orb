...
// Custom hash function for cv::Point3_<float>
struct Point3fHash {
    std::size_t operator()(const cv::Point3f& p) const {
        // Combine the hashes of x, y, and z coordinates to produce a unique hash
...
public:
    ...
    // 保存三维点到二维点的映射关系
    std::map<std::string, std::vector<std::pair<int, int>>> point3d_to_2d_map;// wanglian 
    // Member variable to store the point ID map
