...
// 维护全局三维点数组
// Function to generate and return the PointIDMap
void System::GeneratePointIDMap() {
    // Get all map points
    Map* pActiveMap = mpAtlas->GetCurrentMap();
    if (!pActiveMap)
        cout << "Error: No active map!" << endl;

    const std::vector<MapPoint*>& vpMapPoints = pActiveMap->GetAllMapPoints();

    // Traverse all map points
    int id = 1;
    for (const auto& pMP : vpMapPoints) {
        if (pMP && !pMP->isBad()) {
            // Get the world coordinates of the map point
            Eigen::Vector3f eigenPos = pMP->GetWorldPos();
            // Use Eigen::Vector3f instead of Eigen::Matrix<float, 3, 1>
            cv::Point3f pos(eigenPos.x(), eigenPos.y(), eigenPos.z());
            // Convert the position to a string
            std::ostringstream oss;
            oss  << std::fixed << std::setprecision(20) << pos.x << "," << pos.y << "," << pos.z;
            std::string key = oss.str();
            // Add the mapping from position string to ID to the map
            Point3d_IDMap[key] = id++;
        }
    }
}

void System::SaveKeyPointsAndMapPoints(const std::string &filename) {

    // save the 3d point map<point3d,point3d_id>
    System::GeneratePointIDMap();

    std::cout << std::endl << "Saving key points and map points to " << filename << " ..." << std::endl;
    ...
    cout<<" the number of the keyframes is : "<<vpKFs.size()<<endl;
    ...
    for (size_t i = 0; i < vpKFs.size(); ++i) {
        ...
        std::string timestampStr = oss_time.str();

        f << " " << std::setprecision(7) << timestampStr + ".png" << std::endl;

        int feature_id = 1;
        for (size_t j = 0; j < keypoints.size(); ++j) {
            MapPoint* mp = pKF->GetMapPoint(j);
            if (mp && !mp->isBad()) {
                Eigen::Vector3f eigenPos = mp->GetWorldPos();
                cv::Point3f pos(eigenPos.x(), eigenPos.y(), eigenPos.z());

                // Convert the position to a string
                std::ostringstream oss_3d;
                oss_3d << std::fixed << std::setprecision(20) << pos.x << "," << pos.y << "," << pos.z;
                std::string key = oss_3d.str();

                f << std::setprecision(20) << keypoints[j].pt.x << " " << keypoints[j].pt.y 
                  << " " << Point3d_IDMap[key] << " ";
  

                // 判断三维点是否已经存在映射关系
                if (point3d_to_2d_map.find(key) == point3d_to_2d_map.end()) {
                    point3d_to_2d_map[key] = std::vector<std::pair<int, int>>();
                }
...
void System::SavePointcloud(const string &filename) {
    ...
    Map* pActiveMap = mpAtlas->GetCurrentMap();
    if (!pActiveMap) {
        cerr << "Error: No active map found!" << endl;
        return;
    }

    const vector<MapPoint*>& vpMapPoints = pActiveMap->GetAllMapPoints();
    ...
    for (size_t i = 0; i < vpMapPoints.size(); ++i) {
        ...
        if (pMP && !pMP->isBad()) {
            // Get the world coordinates of the map point
            Eigen::Vector3f worldPos = pMP->GetWorldPos().cast<float>();
            cv::Point3f pos(worldPos.x(), worldPos.y(), worldPos.z());

            // Search for the map point's ID
            std::ostringstream oss_3d;
            oss_3d << std::fixed << std::setprecision(20) << pos.x << "," << pos.y << "," << pos.z;
            std::string key_3d = oss_3d.str();

            int pint3d_id = Point3d_IDMap[key_3d]; // Get the ID from the map
...
float System::CalculateReprojectionErrorForMapPoint(MapPoint* pMP)
{
    // Initialize variables to accumulate reprojection error
    double totalReprojError = 0.0;
    int count = 0;

    // Get observations of the map point
    ...
    for (const auto& obs : observations)
    {
        ...
        auto [idx, subIdx] = obs.second;  // Adjust according to actual tuple structure

        // Skip if the keyframe is bad or index is out of bounds
        if (pKF->isBad() || idx >= pKF->mvKeysUn.size()) continue;

        // Get inverse camera pose of the keyframe
        Sophus::SE3f Twc = pKF->GetPoseInverse();
        ...
        Eigen::Vector3f cameraPos = Twc * worldPos;

        // Project into image plane
        float invZ = 1.0f / cameraPos[2];
        float u = fx * cameraPos[0] * invZ + cx;
        float v = fy * cameraPos[1] * invZ + cy;

        cv::Point2f reprojection(u, v);
        cv::Point2f observed = pKF->mvKeysUn[idx].pt;

        // Calculate reprojection error and accumulate
        double reprojError = cv::norm(reprojection - observed);
        totalReprojError += reprojError;
        count++;
    }
    ...
    if (count > 0)
    {
        return static_cast<float>(totalReprojError / count
        );
    }
    else
    {
