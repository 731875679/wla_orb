        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    int currentMnId = mpCurrentKeyFrame->mnId;

                    // 初始化哈希表时确保插入第一帧和第二帧的位姿信息
                    if (mKeyFrameData.empty()) {
                        std::vector<KeyFrame*> allKFs = mpAtlas->GetAllKeyFrames();
                        for (auto pKF : allKFs) {
                            if (pKF && pKF->mnId <= currentMnId) {
                            currentMnId = pKF->mnId;
                            // 提取关键帧数据
                            cv::Mat currentrgb = mpTracker->mnIdToRGBMap[currentMnId];
                            sensor_msgs::Image currentimageMsg = CreateRosImage(currentrgb, currentMnId);

                            geometry_msgs::PoseStamped currentposeMsg = CreatePoseMessage(pKF);

                            sensor_msgs::PointCloud2 currentmapPointsMsg= createMapPoints(pKF, currentrgb);

                            mKeyFrameData[currentMnId] = std::make_tuple(currentimageMsg, currentposeMsg, currentmapPointsMsg);
                    ...
                    geometry_msgs::PoseStamped currentposeMsg = CreatePoseMessage(mpCurrentKeyFrame);

                    sensor_msgs::PointCloud2 currentmapPointsMsg= createMapPoints(mpCurrentKeyFrame, currentrgb);

                    mKeyFrameData[currentMnId] = std::make_tuple(currentimageMsg, currentposeMsg, currentmapPointsMsg);

                    // 清理 Tracking 中的 RGB 数据
                    for (auto it = mpTracker->mnIdToRGBMap.begin(); it != mpTracker->mnIdToRGBMap.end(); )
                    {
                        if (it->first < currentMnId) {
                            it = mpTracker->mnIdToRGBMap.erase(it);
                        } else {
                            ++it;
                        }
                    }
